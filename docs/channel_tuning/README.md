...
1. 用 `generate_channel_tubes_v2.py` 生成候选 Tube；
2. 用 `evaluate_channel_tubes.py` 检查闭合、曲率、footprint 和地图净空；
3. 用 `generate_channel_connectors_v2.py` 生成路径连接段，并用 `evaluate_channel_connectors.py` 复核；
4. 用 `generate_channel_cone_avoid_map.py` 和 `evaluate_channel_avoid_map.py` 检查锥桶侧向选择；
5. 用 `channel_shadow_vehicle.yaml` 和 `channel_runtime_tuning.yaml` 做延迟车模分析；
