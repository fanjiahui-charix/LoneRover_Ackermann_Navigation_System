...
第一种是 **Path 本身就不对**。Hybrid-A* 的搜索在代价不足、footprint 偏小、膨胀半径不够或锥桶层没有正确写入 costmap 时，可能找到一条几何上能通、实际却贴着锥桶走的路径。尤其是 A* 搜索只看到“能不能过去”，没有得到足够的障碍余量时，路径会沿着锥桶边缘擦过去。这时优先检查全局/局部 costmap、车辆 footprint、障碍膨胀、锥桶层更新和最小转弯半径。普通区域里，`SmacPlannerHybrid` 直接规划出的绕障路径一般已经够用，先把 Path 修好比盲目调 MPPI 有效。

第二种是 **Path 没问题，但车没有执行好**。如果全局 Path 已经绕开锥桶，实际车仍然跟不上，或者遇到突然出现的障碍时躲不开，就要看局部规划器、速度控制器和底盘响应：控制频率、局部规划时域、跟踪误差、命令延迟、舵机响应、速度/加速度限幅和急停链路都可能造成问题。MPPI 的紧急避障尤其难调，它既要在很短的时域里改变速度和转向，又受真实底盘延迟和最大转角限制。排查时要把全局 Path、局部轨迹、`cmd_vel`、安全输出和底盘反馈放在同一条时间线上看。

简单说，Path 是“准备走哪儿”，Planner 是“怎么生成这条 Path”，Controller 和速度控制器是“怎么把它变成命令”，底盘是“命令有没有真的执行”。这几层的问题要分开判断。
